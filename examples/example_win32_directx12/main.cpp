...
static const int APP_SRV_HEAP_SIZE = 256;
...
struct ExampleDescriptorHeapAllocator
{
  ...
  void Destroy()
  {
    ...
    {
        if (!Heap) {
            fprintf(stderr, "[DX12] ERROR: Descriptor heap not initialized in Alloc!\n");
            fflush(stderr);
            abort();
        }
        if (FreeIndices.Size == 0) {
            fprintf(stderr, "[DX12] ERROR: No free SRV descriptors available in Alloc!\n");
            fflush(stderr);
            abort();
        }
...
ID3D12Resource* LoadTextureRGBA(
  size_t image_width, size_t image_height, const uint8_t* image_data,
...
int main(int, char**)
{
  ...
  HWND hwnd = ::CreateWindowW(wc.lpszClassName,
    L"Dear ImGui Zoomable Image DirectX12 Example", WS_OVERLAPPEDWINDOW,
  ...
  // provide callbacks.
  // (current version of the backend will only allocate one descriptor, future
  // versions will need to allocate more)
  ...
  // Create a DirectX 12 RGBA texture and upload it
  const UINT width = 320, height = 240;
  const size_t pixelSize = 4; // RGBA8
  std::vector<uint8_t> textureData(width * height * pixelSize);
  ...
  for (UINT y = 0; y < height; ++y)
  {
    for (UINT x = 0; x < width; ++x)
    {
      ...
      textureData[idx + 0] = v;   // R
      textureData[idx + 1] = v;   // G
      textureData[idx + 2] = v;   // B
      textureData[idx + 3] = 255; // A
    }
  }

  // We need to pass a D3D12_CPU_DESCRIPTOR_HANDLE in ImTextureID, so make sure
  // it will fit
  ...
  ID3D12Resource* texture = LoadTextureRGBA(
    width, height, textureData.data(), g_pd3dDevice, my_texture_srv_cpu_handle);
  ...
  while (!done)
  {
    ...
    if ((g_SwapChainOccluded && g_pSwapChain->Present(0, DXGI_PRESENT_TEST)
      == DXGI_STATUS_OCCLUDED) || ::IsIconic(hwnd))
    {
      ::Sleep(10);
      continue;
    }
    ...
    ImGui::SetNextWindowSize(imageWindowSize, ImGuiCond_Once);
    {
      ...
      ImGuiImage::Zoomable(my_texture_srv_gpu_handle.ptr, displaySize, &zoomState);
      ImGui::End();
    }
    ...
    g_pd3dCommandList->ClearRenderTargetView(
      g_mainRenderTargetDescriptor[backBufferIdx], colorWithAlpha, 0, nullptr);
  ...
  if (my_texture_srv_cpu_handle.ptr)
    g_pd3dSrvDescHeapAlloc.Free(my_texture_srv_cpu_handle, my_texture_srv_gpu_handle);
...
// Returns true on success, with the SRV CPU handle having an SRV for the newly-
// created texture placed in it (srv_cpu_handle must be a handle in a valid
// descriptor heap)
// Modified from:
  ...
  ID3D12Device* d3d_device, D3D12_CPU_DESCRIPTOR_HANDLE srv_cpu_handle)
{
  ...
  UINT uploadPitch = (image_width * 4 + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u)
  ...
  HRESULT hr = d3d_device->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE,
    &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS(&uploadBuffer));
  ...
  hr = d3d_device->CreateCommandList(
    0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAlloc, NULL, IID_PPV_ARGS(&cmdList));
  IM_ASSERT(SUCCEEDED(hr));
...
bool CreateDeviceD3D(HWND hWnd)
{
  ...
  if (g_pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
        g_frameContext[0].CommandAllocator, nullptr, IID_PPV_ARGS(&g_pd3dCommandList)) != S_OK ||
  ...
  case WM_SIZE:
    if (g_pd3dDevice != nullptr && wParam != SIZE_MINIMIZED)
    {
      ...
      HRESULT result = g_pSwapChain->ResizeBuffers(
        0, (UINT)LOWORD(lParam), (UINT)HIWORD(lParam), desc.Format, desc.Flags);
