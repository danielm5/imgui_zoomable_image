...
int main(int, char**)
{
  ...
  HWND hwnd = ::CreateWindowW(wc.lpszClassName,
    L"Dear ImGui Zoomable Image DirectX11 Example", WS_OVERLAPPEDWINDOW,
  ...
  // Create a DirectX 11 RGBA texture
  ...
  desc.MiscFlags = 0;
  if (FAILED(g_pd3dDevice->CreateTexture2D(&desc, nullptr, &texture)))
  {
    throw std::runtime_error("Failed to create test texture");
  }
  D3D11_MAPPED_SUBRESOURCE mappedTex;
  if (FAILED(g_pd3dDeviceContext->Map(texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedTex)))
  {
  ...
  while (!done)
  {
    ...
    ImGui::SetNextWindowSize(imageWindowSize, ImGuiCond_Once);
    {
      ...
      ImGuiImage::Zoomable((ImTextureID)(intptr_t)textureView, displaySize, &zoomState);
      ImGui::End();
    }
    ...
    g_pd3dDeviceContext->ClearRenderTargetView(g_mainRenderTargetView, colorWithAlpha);
...
bool CreateDeviceD3D(HWND hWnd)
{
  ...
  // and handle fullscreen mode differently. See #8979 for suggestions.
